...
const SpotForm = () => {
    ...
    useEffect(() => {
        ...
        if(!title.length) errors.push('Name is required')
        if(!Number(price)) errors.push('Price is required')
        if(!prevImg.length) errors.push('Preview Image is required')
        setValidationErrors(errors)
    }, [country, city, address, state, lat, lng, description, title, price, prevImg])
    ...
    return (
        <div>
            ...
            <p>Guests will only get your exact address once they booked a reservation.</p>
                ...
                />
                {hasSubmitted && !title && (
                    <label htmlFor='Title' className='field-error'>Name is required</label>
                )}
